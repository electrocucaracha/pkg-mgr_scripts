...
# @file Fly validator
# @brief Validates a Fly command-line client installation.
...
function get_version {
    local version=${PKG_FLY_VERSION-}
    attempt_counter=0
    max_attempts=5

    until [ "$version" ]; do
        url_effective=$(curl -sL -o /dev/null -w '%{url_effective}' "https://github.com/concourse/concourse/releases/latest")
...
if ! command -v fly; then
    error "Concourse CI command line wasn't installed"
fi

info "Checking fly version"
if [ "$(fly --version)" != "$(get_version)" ]; then
    error "fly version installed is different that expected"
fi
