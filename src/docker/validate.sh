...
function test_rootless {
    ...
    if systemctl --user daemon-reload >/dev/null 2>&1; then
        ...
        trap_cmd+="systemctl --user stop docker;"
        ...
        docker_pid="/run/user/1000/docker.pid"
        if [ ! -f "$docker_pid" ] && [ -f "$HOME/.docker/run/docker.pid" ]; then
            docker_pid="$HOME/.docker/run/docker.pid"
        fi
        trap_cmd+="kill -s SIGTERM $(cat "$docker_pid");"
        # shellcheck disable=SC2064
        trap "$trap_cmd" RETURN
    fi
...
[ -f "$HOME/bin/dockerd-rootless-setuptool.sh" ] && test_rootless
