...
function main {
	...
	pkg_mgr_supported['act']="{\"Suse\": \"$PKG_MGR_SUPPORTED\",\"Debian\": \"$PKG_MGR_SUPPORTED\",\"RedHat\": \"$PKG_MGR_SUPPORTED\"}"
	pkg_mgr_supported['aws']="{\"Suse\": \"$PKG_MGR_SUPPORTED\",\"Debian\": \"$PKG_MGR_SUPPORTED\",\"RedHat\": \"$PKG_MGR_SUPPORTED\"}"
	...
	pkg_mgr_supported['yq']="{\"Suse\": \"$PKG_MGR_SUPPORTED\",\"Debian\": \"$PKG_MGR_SUPPORTED\",\"RedHat\": \"$PKG_MGR_SUPPORTED\"}"
	...
	pkg_mgr_supported['crudini']="{\"Suse\": \"$PKG_MGR_PIP_REQUIRED\",\"Debian\": \"$PKG_MGR_PIP_REQUIRED\",\"RedHat\": \"$PKG_MGR_PIP_REQUIRED\"}"
		...
		for pkg in $PKG; do
			...
			if [[ -n ${json_pkg} ]]; then
				...
				"$PKG_MGR_UNSUPPORTED")
					...
					;;
				"$PKG_MGR_SUPPORTED")
					curl -fsSL "https://raw.githubusercontent.com/electrocucaracha/pkg-mgr_scripts/master/src/${pkg}/install.sh" | bash
					;;
				"$PKG_MGR_PIP_REQUIRED")
					if ! command -v pip; then
						curl -fsSL "https://raw.githubusercontent.com/electrocucaracha/pkg-mgr_scripts/master/src/pip/install.sh" | bash
					fi
